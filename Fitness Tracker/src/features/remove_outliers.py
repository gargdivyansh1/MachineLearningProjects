...
# as we are looking for the outliers then we could only the numeric data as the categorical data is just the representation of the person and the type of exercise which he or she is doing
...
#plotting for the single column using the funciton written above 
col = 'acc_x'
dataset = mark_outliers_iqr(df, col)

# now we will be making the graph 
...
df[outliers_columns[:3] + ['label']].plot.hist(by = 'label' , figsize = (20, 8), layout = (3,3), bins = 30, alpha = 0.7, sharex = True)
plt.show()
...
# for now we are just going to change those values with nan 

# using the loc = location 
dataset.loc[dataset['gyr_z_outlier'], 'gyr_z']

# now chaning the values 
...
outliers_removed_df.to_pickle('../../data/interim/02_outliers_removed_cahuvetes.pkl')